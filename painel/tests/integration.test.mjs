...
test('Banco persistente, autenticação, CRUD, exclusão e retiradas atômicas',async t=>{
 ...
 await t.test('Começa vazio e guarda senha derivada, não texto puro',async()=>{
  ...
  const admin=store.db.prepare('SELECT * FROM administradores').get();assert.notEqual(admin.senha_hash,'12345');assert.equal(admin.senha_hash.length,128);
 ...
 await t.test('Persiste após reiniciar o servidor e exige novo login',async()=>{
  await stop();await start();assert.equal((await req('/api/session')).data.admin,false);
 ...
 await t.test('Retirada persiste, reduz saldo e não duplica ao reenviar',async()=>{
  ...
  const retry=await req('/api/retiradas','POST',payload,false);assert.equal(retry.data.id,first.data.id);assert.equal((await req('/api/catalogo')).data.products[0].stock,10.25);
 ...
 await t.test('Falha em um item não desconta os outros; concorrência não deixa saldo negativo',async()=>{
  ...
  assert.equal(partial.status,409);assert.equal(store.get('insumos',product.id).stock,10.25);
